...
public class DataManagerFX extends DataManager {
			...
			try (BufferedReader is = new BufferedReader(new InputStreamReader(jis))) {
				...
				while ((line = is.readLine()) != null) {
					...
					while (!prop.eof()) {
						...
						if (anim.root == -1)
							anim.root = tileid;
						rects.add(createTile(0, tileid, 0, 0, 64, 64).getViewport());
	 ...
	 * @return
	 */
	public ImageView createTile(int layer, int tileid, int tilex, int tiley, int tilewidth, int tileheight) {
		try {
			ImageSet[] tiles = sets.get("tiles");
			for (int i = 0; i < tiles.length; i++) {
				ImageSetFX is = (ImageSetFX)tiles[i];
				if (tileid >= is.gid && tileid < is.gid + is.count) {
					...
					switch (layer) {
						case 0:
							iv = new ImageView(is.tiles[tileid - is.gid]);
							break;
						case 1:
							iv = new ImageView(is.tilesAlpha[tileid - is.gid]);
							break;
						case 2:
							iv = new ImageView(is.tilesAlpha2[tileid - is.gid]);
							break;
					}
					iv.relocate(tilex * tilewidth, tiley * tileheight - (is.height - tileheight));
					return iv;
	...
	public class ImageSetFX extends ImageSet {
		...
		Image image;
		public Image[] tiles;
		public Image[] tilesAlpha;
		public Image[] tilesAlpha2;
		...
		public void load(InputStream is) throws IOException {
			...
			for (int i = 0; i < count; i++) {
				tilesAlpha2[i] = new WritableImage(readerAlpha2, i * width, 0, width, height);
			}
		}
	}
}
