...
public class ClientMap {
	...
	public ClientMap(int cols, int rows) {
		...
		map = new short[cols * rows];
		mapAlpha = new short[cols * rows];
		mapAlpha2 = new short[cols * rows];
		groundLevel = 0;
	}
	...
	public int getTile(int x, int y) {
		return getTile(groundLevel, x, y);
	}
	...
	public int getTile(int level, int x, int y) {
		...
		switch (level) {
			case 0:
				return map[x + y * cols];
			case 1:
				return mapAlpha[x + y * cols];
			case 2:
				return mapAlpha2[x + y * cols];
		}
		return 0;
	}
