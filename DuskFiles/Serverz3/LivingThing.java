...
//		if (privs>1 || (batBattle == null && !blnSleep && engGame.canMoveTo(newLocX,newLocY,this)))
	...
	*/
	...
	public void updateMap()
	{
		...
		send(new DuskMessage.StringMessage(DuskProtocol.MSG_UPDATE_MAP, strResult));
	}
...
//			}
	...
	void loadRace()
	{
		...
		try
		{
			if (strRace == null || !(new File(strRaceDir+"/"+strRace).exists()))
			{
				...
				if (isPet())
				{
					DuskMessage msg = DuskMessage.receiveMessage(thnMaster.stmIn);
					if (msg.name == DuskProtocol.MSG_COMMAND) {
						strStore = ((DuskMessage.StringMessage) msg).value.toLowerCase();
					} else {
						strStore = "";
					}
				} else
	...
	public void updateStats()
	{
		try
		{
			...
			for (int i=0;i<vctConditions.size();i++)
			{
				...
				{
					msg.add(DuskMessage.create(DuskProtocol.FIELD_CONDITIONS, cndStore.strName));
				}
			}
			...
			for (int i=0;i<vctSkills.size();i++)
			{
				...
				msg.add(DuskMessage.create(DuskProtocol.FIELD_SKILLS, sklStore.strName+": "+sklStore.value));
			}
			...
			for (int i=0;i<vctSpells.size();i++)
			{
				...
				if (grpStore != null)
				{
					msg.add(DuskMessage.create(DuskProtocol.FIELD_SPELLS, splStore.strName+": "+splStore.value));
					msg.add(DuskMessage.create(DuskProtocol.FIELD_SPELLS, grpStore.spellList(splStore.value)));
				}
			}
	...
	void resizeMap()
	{
		...
		send(new DuskMessage.StringMessage(DuskProtocol.MSG_INIT_MAP, strResult));
