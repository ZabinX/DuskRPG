...
public class DuskEngine implements Runnable
{
	...
	synchronized void refreshEntities(LivingThing thnRefresh)
	{
									...
									if (objStore.isLivingThing())
									{
										...
										if (blnCanSee)
										{
											...
											if (thnStore.isPlayer())
											{
												...
												msg.add(new DuskMessage.IntegerMessage(0, thnStore.intLocX));
												msg.add(new DuskMessage.IntegerMessage(0, thnStore.intLocY));
												msg.add(new DuskMessage.IntegerMessage(0, thnStore.intImage));
												msg.add(new DuskMessage.IntegerMessage(0, thnStore.intStep));
												thnRefresh.send(msg);
		...
		for (i=0;i<thnRefresh.vctEntities.size();i++)
		{
			...
			thnRefresh.send(new DuskMessage.IntegerMessage(DuskProtocol.MSG_REMOVE_ENTITY, (int)objStore.ID));
		}
