...
public class Dusk implements Runnable,MouseListener,KeyListener,ComponentListener, ImageObserver
{
	...
	public void	run()
	{
		...
		try
		{
			...
			if (msg instanceof DuskMessage.StringMessage) {
				...
				// For simplicity, we'll assume list messages are not used in this switch
			}
			...
			case(0):
			{
		        ...
		        case (DuskProtocol.MSG_MAP):
		        {
					if (msg instanceof MapMessage) {
						...
						{
							shrMap = mapMsg.map;
							shrMapAlpha = mapMsg.mapAlpha;
							shrMapAlpha2 = mapMsg.mapAlpha2;
    ...
    public void sendMessage(DuskMessage message) {
        try {
            if (stmOut != null) {
                ...
                message.sendMessage(dataStream);
                dataStream.flush();
    ...
    public void sendMessage(String message) {
        sendMessage(new DuskMessage.StringMessage(DuskProtocol.MSG_COMMAND, message));
    }
