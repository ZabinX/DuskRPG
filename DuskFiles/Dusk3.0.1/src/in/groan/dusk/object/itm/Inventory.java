...
public class Inventory extends Hashtable implements Constants {
	...
	{
		LifoQueue qTemp;
		if (contains(itmStore.NAME))
		{
			qTemp = (LifoQueue)get(itmStore.NAME);
			qTemp.push(itmStore);
		} else
		{
			qTemp = new LifoQueue();
			qTemp.push(itmStore);
			super.put(itmStore.NAME.toLowerCase(), qTemp);
		}
	}
	...
	*/
	public String print()
	{
		...
		while(iter.hasNext())
		{
			...
			invBuffer.append(""+(char)3).append(qStore.size()).append(" ").append(itmStore.NAME).append("\n");
		}
