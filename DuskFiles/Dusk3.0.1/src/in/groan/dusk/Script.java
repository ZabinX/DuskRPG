...
//		rafScript = new RandomAccessFile(name, "r");
	...
	Entity getLivingThing(String strName)
	{
		try
		{
			Entity thnStore;
			...
			}else if (strName.equalsIgnoreCase("enemy"))
			{
				...
				if (thnStore.bytSide == 0)
					...
					return (Entity)thnStore.BATTLE.vctSide2.elementAt(0);
				else if (thnStore.bytSide == 2)
					return (Entity)thnStore.BATTLE.vctSide1.elementAt(0);
			}else if (strName.equalsIgnoreCase("pet"))
			{
				thnStore = getLivingThing(getString());
				if (thnStore.ENT_FOLLOWING != null && thnStore.ENT_FOLLOWING.isPet())
				{
					return thnStore.ENT_FOLLOWING;
				}
				return null;
			...
			}else if (strName.equalsIgnoreCase("global"))
			{
				...
				if (varStore != null && varStore.isLivingThing())
					return (Entity)varStore.objData;
			}
	...
	public void runScript()
	{
		...
		{
			thnStore = (Entity)vctUpdate.elementAt(i);
			thnStore.updateStats();
		}
		for (int i=0;i<vctVisibleUpdate.size();i++)
		{
			thnStore = (Entity)vctVisibleUpdate.elementAt(i);
		...
		if (strStore.startsWith("\""))
		{
			...
			while (true)
			{
				...
				}else if (c == '\r')
				{
					strStore += "\n";
					intLine++;
					// Skip over LF is this was a CR/LF pair
					c2 = (char)rafScript.readByte();
					if (c2 != '\n')
					{
...
//		long lngOldFilePointer;
		...
		if (strStore.equalsIgnoreCase("name"))
		{
			Entity thnStore = getLivingThing(getString());
			if (thnStore != null)
			{
	...
	boolean parseScript()
	{
        ...
        try
        {
			...
			switch (bytCommand)
			{
        		...
        		case 26:
          		{
	                if(getLivingThing(getString()).GUILD.equalsIgnoreCase(getString()))
	                	return true;
	                return false;
            	}
            	case 27:
            	{
            		if(getLivingThing(getString()).STR_RACE.equalsIgnoreCase(getString()))
            			return true;
            	    return false;
            	}
            	case 28:
            	...
            	{	
            		Entity thnStore = getLivingThing(getString());
            		thnStore.unequipItem(getString());
            	    thnStore.updateItems();
            	    return true;
           		}
            	case 34:
            	{
            		return false;//getLivingThing(getString()).isWearing(getString());
            	}
            	case 35:
            	{
            		getLivingThing(getString()).unequipItem(getString());
                	return true;
            	}
            	case 36:
            	{	
            	...
            	case 42:
    			{
    				...
    				while (intIndex != -1)
    				{
    					thnStore.chatMessage(strStore2.substring(0,intIndex+1));
            	...
            	case 43:
    			{
    				...
    				while (intIndex != -1)
    				{
    					thnStore.chatMessage(red,green,blue,strStore2.substring(0,intIndex+1));
    					strStore2 = strStore2.substring(intIndex+1);
    					intIndex = strStore2.indexOf("\n");
    				}
            	...
            	case 44:
            	{
            	    ...
            	    case 22:
                	{
                		thnStore.CUR_EXP += (int)parseValue();
                		if (vctUpdate != null || vctUpdate.contains(thnStore))
                			vctUpdate.addElement(thnStore);
    					return true;
            		}
...
//					engGame.changeMap((int)parseValue(),(int)parseValue(),(short)parseValue());
					engGame.changeMap(0,dbgX,dbgY,dbgValue);
					return true;
            	}
            	case 47:
            	{
            	...
            	case 48:
            	{
					...
					thnStore.THR_CONN.sleep(750);  // wait for it...
					try
					{
					...
					strInput = thnStore.BUF_SCK_IN.readLine();
				...
				case 53:
				{
					...
					if(thnStore.isPlayer())
					{
						if (engGame.GP_LOSE_MOD != 0)
						{
							int gplost = (int)(thnStore.ZENY * engGame.GP_LOSE_MOD);
							thnStore.chatMessage("You have lost "+gplost+"gp\n");
							thnStore.ZENY -= gplost;
						}
						if (engGame.EXP_LOSE_MOD != 0)
						{
							int explost = (int)(thnStore.CUR_EXP * engGame.EXP_LOSE_MOD);
							thnStore.chatMessage("You have lost "+explost+"exp\n");
							thnStore.CUR_EXP -= explost;
						}
    					engGame.chatMessage(thnStore.NAME+" has been killed by "+strWhat,"default");
						if (engGame.SCR_ONDEATH != null && thnStore.BLN_WORKING)
						{
							engGame.SCR_ONDEATH.varVariables.clearVariables();
							engGame.SCR_ONDEATH.varVariables.addVariable("trigger",thnStore);
				...
				case 54:
				{
            		Entity thnStore = getLivingThing("trigger");
					if (thnStore != null && thnStore.isLivingThing())
					{
						thnStore.BLN_CONT_TO_CMD = false;
					}
					return true;
				}
				case 55:
            	...
            	case 58:
    			{
    				...
    				while (intIndex != -1)
    				{
            			if(thnStore.BATTLE != null && thnStore.BLN_POPUP)
						{
	...
	private double parseValue()
	{
		try
		{
			...
			//player qualities:
			Entity target = getLivingThing(strStore);
			strStore = getString();
			...
			if (strStore.equalsIgnoreCase("cash")) return (target.ZENY);
			if (strStore.equalsIgnoreCase("exp")) return (target.CUR_EXP);
			if (strStore.equalsIgnoreCase("locx")) return (target.intLocX);
			if (strStore.equalsIgnoreCase("locy")) return (target.intLocY);
	//		if (strStore.equalsIgnoreCase("hp")) return (target.S);
