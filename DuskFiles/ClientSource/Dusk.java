...
{
	private int loginState = 0; // 0=username, 1=password, 2=in-game
	    ...
	    try
	    {
	    	...
	    	if (nkey == KeyEvent.VK_ENTER)
	    	{
	    		...
	    		if(strStore.startsWith("!"))
	    		{
					...
					loginUsername = strStore;
					loginState = 1;
					addText("Password: \n");
				} else if (loginState == 1) {
					ListMessage msg = new ListMessage(DuskProtocol.MSG_AUTH);
					msg.add(new StringMessage(DuskProtocol.FIELD_AUTH_PLAYER, loginUsername));
					msg.add(new StringMessage(DuskProtocol.FIELD_AUTH_PASS, strStore));
					sendMessage(msg);
					loginState = 2; // Move to in-game state
				} else
				{
