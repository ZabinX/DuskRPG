...
/*c*/var C$=Clazz.newClass(P$, "ByteBandedRaster", null, 'sun.awt.image.SunWritableRaster');
...
C$.$fields$=[['I',['scanlineStride','maxX','maxY'],'O',['dataOffsets','int[]','data','byte[][]']]]

Clazz.newMeth(C$, 'c$$java_awt_image_SampleModel$java_awt_Point',  function (sampleModel, origin) {
C$.c$$java_awt_image_SampleModel$java_awt_image_DataBuffer$java_awt_Rectangle$java_awt_Point$sun_awt_image_ByteBandedRaster.apply(this, [sampleModel, sampleModel.createDataBuffer$(), Clazz.new_([origin.x, origin.y, sampleModel.getWidth$(), sampleModel.getHeight$()],$I$(1,1).c$$I$I$I$I), origin, null]);
}, 1);
...
C$.c$$java_awt_image_SampleModel$java_awt_image_DataBuffer$java_awt_Rectangle$java_awt_Point$sun_awt_image_ByteBandedRaster.apply(this, [sampleModel, dataBuffer, Clazz.new_([origin.x, origin.y, sampleModel.getWidth$(), sampleModel.getHeight$()],$I$(1,1).c$$I$I$I$I), origin, null]);
}, 1);

Clazz.newMeth(C$, 'c$$java_awt_image_SampleModel$java_awt_image_DataBuffer$java_awt_Rectangle$java_awt_Point$sun_awt_image_ByteBandedRaster',  function (sampleModel, dataBuffer, aRegion, origin, parent) {
...
throw Clazz.new_(Clazz.load('java.awt.image.RasterFormatException').c$$S,["ByteBandedRaster must havebyte DataBuffers"]);
}var dbb=dataBuffer;
...
var dOffsets=dbb.getOffsets$();
this.dataOffsets=Clazz.array(Integer.TYPE, [bankIndices.length]);
this.data=Clazz.array(Byte.TYPE, [bankIndices.length, null]);
var xOffset=aRegion.x - origin.x;
var yOffset=aRegion.y - origin.y;
for (var i=0; i < bankIndices.length; i++) {
this.data[i]=$I$(2).stealData$java_awt_image_DataBufferByte$I(dbb, bankIndices[i]);
this.dataOffsets[i]=dOffsets[bankIndices[i]] + xOffset + yOffset * this.scanlineStride  + bandOffsets[i];
}
} else {
throw Clazz.new_(Clazz.load('java.awt.image.RasterFormatException').c$$S,["ByteBandedRasters must haveBandedSampleModels"]);
...
var deltaX=x0 - x;
var deltaY=y0 - y;
return Clazz.new_(C$.c$$java_awt_image_SampleModel$java_awt_image_DataBuffer$java_awt_Rectangle$java_awt_Point$sun_awt_image_ByteBandedRaster,[sm, this.dataBuffer, Clazz.new_($I$(1,1).c$$I$I$I$I,[x0, y0, width, height]), Clazz.new_($I$(3,1).c$$I$I,[this.sampleModelTranslateX + deltaX, this.sampleModelTranslateY + deltaY]), this]);
});
...
if (this.data[0].length <= maxIndex * this.numDataElements) {
throw Clazz.new_(Clazz.load('java.awt.image.RasterFormatException').c$$S,["Data array too small (it is " + this.data[0].length + " and should be > " + (maxIndex * this.numDataElements) + " )" ]);
}} else {
for (var i=0; i < this.numDataElements; i++) {
if (this.data[i].length <= maxIndex) {
throw Clazz.new_(Clazz.load('java.awt.image.RasterFormatException').c$$S,["Data array too small (it is " + this.data[i].length + " and should be > " + maxIndex + " )" ]);
}}
}}, p$1);

Clazz.newMeth(C$, 'toString',  function () {
return  String.instantialize("ByteBandedRaster: width = " + this.width + " height = " + this.height + " #bands " + this.numDataElements + " minX = " + this.minX + " minY = " + this.minY );
});
