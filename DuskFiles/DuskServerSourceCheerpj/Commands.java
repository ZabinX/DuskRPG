	...
	public static String parseCommand(LivingThing lt, DuskEngine engGame, String strStore) throws Exception
	{
		...
		*/
		if (strCommand.substring(0,1) != "/")
		{
			try
			{
				Script scrStore = new Script("/app/commands/"+strCommand,engGame,false);
		...
		if (lt.privs > 2)
		{
			if (lt.privs > 4)
			{
				...
				}else if (strCommand.equals("ipban"))
				{
					...
					try
					{
						RandomAccessFile rafBannedIP = new RandomAccessFile("/app/conf/blockedIP","r");
						...
						lt.rafFile.close();
						rafBannedIP = new RandomAccessFile("/app/conf/blockedIP","rw");
			...
			}else if (strCommand.equals("view"))
			{
				...
				if (strArgs.startsWith("item "))
				{
					...
					strFileName = "/app/defItems/"+strArgs.substring(5);
				}else if (strArgs.startsWith("conf "))
				{
					strFileName = "/app/conf/"+strArgs.substring(5);
				}else if (strArgs.startsWith("mob "))
				{
					strFileName = "/app/defMobs/"+strArgs.substring(4);
				}else if (strArgs.startsWith("command "))
				{
					strFileName = "/app/commands/"+strArgs.substring(8);
				}else if (strArgs.startsWith("race "))
				{
					strFileName = "/app/defRaces/"+strArgs.substring(5);
				}else if (strArgs.startsWith("pet "))
				{
					strFileName = "/app/defPets/"+strArgs.substring(5);
				}else if (strArgs.startsWith("faction"))
				{
					return "You cannot view faction files.";
				}else if (strArgs.startsWith("condition "))
				{
					strFileName = "/app/defConditions/"+strArgs.substring(10);
				}else if (strArgs.startsWith("help "))
				{
					strFileName = "/app/helpFiles/"+strArgs.substring(5);
				}else if (strArgs.startsWith("script "))
				{
					strFileName = "/app/scripts/"+strArgs.substring(7);
				}else if (strArgs.startsWith("spell group "))
				{
					strFileName = "/app/defSpellGroups/"+strArgs.substring(12);
				}else if (strArgs.startsWith("spell "))
				{
					strFileName = "/app/defSpells/"+strArgs.substring(6);
				}else if (strArgs.startsWith("prop "))
				{
					strFileName = "/app/defProps/"+strArgs.substring(5);
				}else if (strArgs.startsWith("move action "))
				{
					strFileName = "/app/defMoveActions/"+strArgs.substring(12);
				}else if (strArgs.startsWith("can move "))
				{
					strFileName = "/app/defCanMoveScripts/"+strArgs.substring(9);
				}else if (strArgs.startsWith("can see "))
				{
					strFileName = "/app/defCanSeeScripts/"+strArgs.substring(8);
				}else if (strArgs.startsWith("tile action "))
				{
					strFileName = "/app/defTileActions/"+strArgs.substring(12);
				}else if (strArgs.startsWith("tile move "))
				{
					strFileName = "/app/defTileScripts/"+strArgs.substring(10);
				}else if (strArgs.startsWith("tile see "))
				{
					strFileName = "/app/defTileSeeScripts/"+strArgs.substring(9);
				}else if (strArgs.startsWith("user "))
				{
					...
					blnUser = true;
					strFileName = "/app/users/"+strArgs.substring(5);
				}else if (strArgs.startsWith("pet "))
				{
					if (lt.privs < 5)
					{
					...
					blnPet = true;
					strFileName = "/app/pets/"+strArgs.substring(4);
				}
			...
			}else if (strCommand.equals("submit"))
			{
				...
				{
					strFileName = "/app/defPets/"+strArgs.substring(4);
			...
			}else if (strCommand.equals("delete"))
			{
				...
				{
					strFileName = "/app/defConditions/"+strArgs.substring(8);
...
//				lt.chatMessage(Commands.parseCommand(thnStore, engGame, strArgs));
		...
		if (strCommand.equals("help"))
		{
			...
			synchronized(lt.stmOut)
			{
				try
				{
					if (strArgs == null)
					{
						rafHelp = new RandomAccessFile("/app/help","r");
						if (lt.popup)
						{
					...
					}else
					{
						...
						rafHelp = new RandomAccessFile("/app/helpFiles/"+strArgs,"r");
						if (lt.popup)
						{
		...
		if (strCommand.equals("get"))
		{
			...
			if (itmStore.strOnGetScript != null)
			{
				try
				{
					Script scrStore = new Script("/app/scripts/"+itmStore.strOnGetScript,engGame,false);
					scrStore.varVariables.addVariable("trigger",lt);
		...
		if (strCommand.equals("drop"))
		{
			...
			if (itmStore != null)
			{
				...
				if (itmStore.strOnDropScript != null)
				{
					try
					{
						scrStore = new Script("/app/scripts/"+itmStore.strOnDropScript,engGame,false);
						scrStore.varVariables.addVariable("trigger",lt);
						scrStore.varVariables.addVariable("itemname",itmStore.strName);
						scrStore.varVariables.addVariable("mod",itmStore.intMod);
					}catch(Exception e) {}
		...
		if (strCommand.equals("give"))
		{
			...
			if (itmStore != null)
			{
				...
				if (itmStore.strOnDropScript != null)
				{
					try
					{
						scrDropStore = new Script("/app/scripts/"+itmStore.strOnDropScript,engGame,false);
					}catch(Exception e) {}
				}
				if (itmStore.strOnGetScript != null)
				{
					try
					{
						scrGetStore = new Script("/app/scripts/"+itmStore.strOnGetScript,engGame,false);
					}catch(Exception e) {}
				}
				...
				if (thnStore.strName.equalsIgnoreCase(strArgs))
				{
					...
					try
					{
						if (lt.bfrSocketIn.readLine().equalsIgnoreCase("yes"))
						{
							...
							File deleteme = new File("/app/pets/"+lt.strName.toLowerCase());
							deleteme.delete();
							deleteme = new File("/app/pets/"+lt.strName.toLowerCase()+".backup");
							deleteme.delete();
							lt.thnFollowing.close();
