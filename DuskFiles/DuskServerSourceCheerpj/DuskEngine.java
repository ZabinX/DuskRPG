...
public class DuskEngine implements Runnable
{
	...
	public DuskEngine()
	{
		...
		try 
		{
			...
			// Load Map
			File newmap = new File("/app/shortmap");
			if (newmap.exists())
			{
				rafFile = new RandomAccessFile("/app/shortmap", "r");
			...
			{
				rafFile = new RandomAccessFile("/app/map", "r");
			...
			File mapprivs = new File("/app/shortmap.privs");
			if (mapprivs.exists())
			{
				rafFile = new RandomAccessFile("/app/shortmap.privs", "r");
			...
			File mapowners = new File("/app/shortmap.owner");
			File ownerlist = new File("/app/tile_owner_list");
			if (mapowners.exists() && ownerlist.exists())
			{
				IDtoName = new Config(this, "/app/tile_owner_list");
				rafFile = new RandomAccessFile("/app/shortmap.owner", "r");
		    ...
		    try
		    {
		    rafFile = new RandomAccessFile("/app/merchants", "r");
		    Merchant mrcStore;
			...
			while (!(strStore == null || strStore.equals("")))
			{
				...
				log.printMessage(Log.VERBOSE,"/app/Merchant("+mrcStore.intLocX+","+mrcStore.intLocY+")");
				strStore = rafFile.readLine();
			...
			rafFile = new RandomAccessFile("/app/mobs", "r");
			...
			// Load signs
			Sign sgnStore;
			rafFile = new RandomAccessFile("/app/signs", "r");
			...
			// Load props
			Prop prpStore;
			rafFile = new RandomAccessFile("/app/props","r");
			...
			String strObject;
			double dblObject;
			rafFile = new RandomAccessFile("/app/globals", "r");
			...
			try
			{
				Script scrOnBoot = new Script("/app/conf/onBoot",this,true);
				scrOnBoot.runScript();
				scrOnBoot.close();
	...
	boolean isGoodIP(String strIP)
	{
		...
		try
		{
			...
			rafFile = new RandomAccessFile("/app/conf/blockedIP", "r");
	...
	boolean isGoodName(String strName)
	{
		...
		try
		{
			...
			rafFile = new RandomAccessFile("/app/conf/dirtyWordFile", "r");
	...
	{
		Config settings = new Config(this, "/app/conf/prefs");
		//Load Prefs
		...
		try
		{
			if (scrCanSeeLivingThing != null)
			{
				synchronized(scrCanSeeLivingThing)
				{
					...
					scrCanSeeLivingThing = new Script("/app/conf/canSeeLivingThing",this,true);
				}
			} else
			{
				scrCanSeeLivingThing = new Script("/app/conf/canSeeLivingThing",this,true);
		...
		try
		{
			if (scrCanMoveThroughLivingThing != null)
			{
				synchronized(scrCanMoveThroughLivingThing)
				{
					...
					scrCanMoveThroughLivingThing= new Script("/app/conf/canMoveThroughLivingThing",this,true);
				}
			}else
			{
				scrCanMoveThroughLivingThing= new Script("/app/conf/canMoveThroughLivingThing",this,true);
		...
		try
		{
			if (scrCanAttack != null)
			{
				...
				{
					scrCanAttack.close();
					scrCanAttack = new Script("/app/conf/canAttack",this,true);
				}
			} else
			{
				scrCanAttack = new Script("/app/conf/canAttack",this,true);
		...
		try
		{
			if (scrOnStart != null)
			{
				...
				{
					scrOnStart.close();
					scrOnStart = new Script("/app/conf/onStart",this,true);
				}
			} else
			{
				scrOnStart = new Script("/app/conf/onStart",this,true);
		...
		try
		{
			if (scrOnDeath != null)
			{
				...
				{
					scrOnDeath.close();
					scrOnDeath = new Script("/app/conf/onDeath",this,true);
				}
			} else
			{
				scrOnDeath = new Script("/app/conf/onDeath",this,true);
		...
		try
		{
			if (scrOnLogOut != null)
			{
				...
				{
					scrOnLogOut.close();
					scrOnLogOut = new Script("/app/conf/onLogOut",this,true);
				}
			} else
			{
				scrOnLogOut = new Script("/app/conf/onLogOut",this,true);
		...
		synchronized(vctTiles)
		{
			try
			{
				...
				for (i=0;true;i++)
				{
					try
					{
						vctTiles.addElement(new Script("/app/defTileScripts/"+i,this,true));
					}catch(Exception e)
					{
						break;
					}
		...
		synchronized(vctTileActions)
		{
			try
			{
				...
				for (i=0;true;i++)
				{
					try
					{
						vctTileActions.addElement(new Script("/app/defTileActions/"+i,this,true));
					}catch(Exception e)
					{
						break;
					}
		...
		synchronized(vctSeeTiles)
		{
			try
			{
				...
				for (i=0;true;i++)
				{
					try
					{
						vctSeeTiles.addElement(new Script("/app/defTileSeeScripts/"+i,this,true));
					}catch(Exception e)
					{
						break;
					}
...
//			pla2.batBattle.chatMessage("\t"+pla1.strName+" has joined the battle.");
	...
	Prop getProp(String strName)
	{
		...
		try
		{
			RandomAccessFile rafPropDef = new RandomAccessFile("/app/defProps/"+strName, "r");
	...
	Condition getCondition(String inName)
	{
		...
		try
		{
			RandomAccessFile rafConditionDef = new RandomAccessFile("/app/defConditions/"+inName.toLowerCase(), "r");
	...
	int getSpellPercent(String strName)
	{
		try
		{
			RandomAccessFile rafSpell = new RandomAccessFile("/app/defSpells/"+strName,"r");
	...
	SpellGroup getSpellGroup(String strName)
	{
		...
		try
		{
			...
			RandomAccessFile rafSpellGroup = new RandomAccessFile("/app/defSpellGroups/"+strName,"r");
			try
			{
	...
	Item getItem(String inName)
	{
		...
		try
		{
			File fileTest = new File("/app/defItems/"+inName.toLowerCase());
			if (!fileTest.exists())
			{
				return null;
			}
			rafItemDef = new RandomAccessFile("/app/defItems/"+inName.toLowerCase(), "r");
	...
	boolean canMoveTo(int inLocX, int inLocY,LivingThing thnStore)
	{
		...
		try
		{
			scrStore = new Script("/app/defCanMoveScripts/"+inLocX+"_"+inLocY,this,false);
	...
	boolean canSee(int inLocX, int inLocY,LivingThing thnStore)
	{
		...
		try
		{
			scrStore = new Script("/app/defCanSeeScripts/"+inLocX+"_"+inLocY,this,false);
			scrStore.varVariables.addVariable("trigger",thnStore);
			blnStore = scrStore.rewindAndParseScript();
			scrStore.close();
			return blnStore;
	...
	void saveMap()
	{
		...
		try 
		{
			...
			if (blnMapHasChanged)
			{
				...
				synchronized(shrMap)
				{
					deleteme = new File("/app/shortmap");
					deleteme.delete();
					rafFile = new RandomAccessFile("/app/shortmap", "rw");
				...
				String strMapLog = "/app/shortmap_redraw";
			...
			if (blnMobListChanged)
			{
				...
				{
					deleteme = new File("/app/mobs");
					deleteme.delete();
		    		rafFile = new RandomAccessFile("/app/mobs", "rw");
			...
			if (blnSignListChanged)
			{
				...
				{
					deleteme = new File("/app/signs");
					deleteme.delete();
		    		rafFile = new RandomAccessFile("/app/signs", "rw");
					for (i=0;i<vctSigns.size();i++)
					{
						sgnStore = (Sign)vctSigns.elementAt(i);
			...
			if (blnMerchantListChanged)
			{
				...
				{
					deleteme = new File("/app/merchants");
					deleteme.delete();
		    		rafFile = new RandomAccessFile("/app/merchants", "rw");
			...
			if (blnPropListChanged)
			{
				...
				{
					deleteme = new File("/app/props");
					deleteme.delete();
		    		rafFile = new RandomAccessFile("/app/props", "rw");
			...
			if (blnVariableListChanged)
			{
				...
				{
					deleteme = new File("/app/globals");
					deleteme.delete();
		    		rafFile = new RandomAccessFile("/app/globals", "rw");
					Variable varStore;
	...
	void backupMap()
	{
		try 
		{
			...
			synchronized(shrMap)
			{
			File deleteme = new File("/app/backup/shortmap.backup");
			deleteme.delete();
			RandomAccessFile rafFile = new RandomAccessFile("/app/backup/shortmap.backup", "rw");
		    int i,
		        i2;
		    rafFile.writeInt(MapColumns);
		    rafFile.writeInt(MapRows);
			...
			deleteme = new File("/app/backup/mobs.backup");
			deleteme.delete();
		    rafFile = new RandomAccessFile("/app/backup/mobs.backup", "rw");
			for (i=0;i<vctMobs.size();i++)
			{
				mobStore = (Mob)vctMobs.elementAt(i);
				if (mobStore.blnOneUse == false)
			...
			deleteme = new File("/app/backup/signs.backup");
			deleteme.delete();
		    rafFile = new RandomAccessFile("/app/backup/signs.backup", "rw");
			for (i=0;i<vctSigns.size();i++)
			{
				sgnStore = (Sign)vctSigns.elementAt(i);
				rafFile.writeBytes(sgnStore.strMessage+"\n"+sgnStore.intLocX+"\n"+sgnStore.intLocY+"\n");
			...
			deleteme = new File("/app/backup/merchants.backup");
			deleteme.delete();
		    rafFile = new RandomAccessFile("/app/backup/merchants.backup", "rw");
			for (i=0;i<vctMerchants.size();i++)
			{
				mrcStore = (Merchant)vctMerchants.elementAt(i);
				rafFile.writeBytes(mrcStore.intLocX+"\n"+mrcStore.intLocY+"\n");
			...
			deleteme = new File("/app/backup/props.backup");
			deleteme.delete();
		    rafFile = new RandomAccessFile("/app/backup/props.backup", "rw");
			for (i=0;i<vctProps.size();i++)
			{
				prpStore = (Prop)vctProps.elementAt(i);
				rafFile.writeBytes(prpStore.strName+"\n"+prpStore.intLocX+"\n"+prpStore.intLocY+"\n");
