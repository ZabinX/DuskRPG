...
public class Merchant extends DuskObject
{
	...
	void pet(LivingThing thnMaster)
	{
		try
		{
			...
			File filCheck = new File("/app/pets/"+strName.toLowerCase());
			File filCheck2 = new File("/app/users/"+strName.toLowerCase());
					...
					)
			{
				...
				filCheck = new File("/app/pets/"+strName.toLowerCase());
				filCheck2 = new File("/app/users/"+strName.toLowerCase());
			}
			thnMaster.chatMessage("Choose a type of pet:");
			File filPetTypes = new File("/app/defPets");
			...
			filCheck = new File("/app/defPets/"+strStore);
