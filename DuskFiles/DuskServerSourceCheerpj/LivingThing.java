...
public class LivingThing extends DuskObject implements Runnable
{
	...
	*/
		...
		try
		{
			...
			*/
			...
			if (strName.equalsIgnoreCase("default"))
			{
				...
				filCheck = new File("/app/pets/"+strStore);
			...
			rafFile = new RandomAccessFile("/app/pets/default", "rw");
			...
			rafFile.close();
			strStore = thnMaster.strName.toLowerCase();
			File filPlayer = new File("/app/pets/"+strStore);
			File filBackup = new File("/app/pets/"+strStore+".backup");
			/*
			...
			*/
			if (!strName.equalsIgnoreCase("default"))
			{
				...
				File filOldPet = new File("/app/pets/"+strStore);
				File filOldBackup = new File("/app/pets/"+strStore+".backup");
			...
			if (filBackup.exists())
			{
				...
				{
					filCheck = new File("/app/backup/"+strStore+".possiblyDamaged");
					while (filCheck.exists())
					{
						i++;
						filCheck = new File("/app/backup/"+strStore+".possiblyDamaged."+i);
					}
				...
				}else if (filPlayer.length() < filBackup.length())
				{
					...
					filBackup.renameTo(new File("/app/pets/"+strStore));
				}
			}
			strStore = thnMaster.strName.toLowerCase();
			rafFile = new RandomAccessFile("/app/pets/"+strStore,"rw");
			rafBackup = new RandomAccessFile("/app/pets/"+strStore+".backup","rw");
			strStore = rafFile.readLine();
			while (!(strStore == null || strStore.equals(".")))
			{
				parseUserFile(strStore);
			...
			if (strRace != null)
			{
				RandomAccessFile rafRaceFile = new RandomAccessFile("/app/defPets/"+strRace.toLowerCase(),"r");
	...
	boolean addToSkill(String sklName, int add)
	{
		...
		File filCheck = new File("/app/defSpellGroups/"+sklName);
		if (filCheck.exists())
		{
			if (add > 100)
			{
...
//		if (privs>1 || (batBattle == null && !blnSleep && engGame.canMoveTo(newLocX,newLocY,this)))
		...
		try
		{
			...
			try
			{
				scrStore = new Script("/app/defMoveActions/"+intLocX+"_"+intLocY,engGame,false);
				scrStore.varVariables.addVariable("trigger",this);
				scrStore.runScript();
				scrStore.close();
			}catch (Exception e){}
	...
	*/
	...
	synchronized public void changeLocBypass(int newLocX, int newLocY)
	{
		...
		try
		{
			...
			try
			{
				scrStore = new Script("/app/defMoveActions/"+newLocX+"_"+newLocY,engGame,false);
	    		scrStore.varVariables.addVariable("trigger",this);
	    		scrStore.runScript();
	    		scrStore.close();
	    	}catch (Exception e){}
	...
	void useItem(String strStore, int mustBeType)
	{
		...
		try
		{
			Script scrStore = new Script("/app/scripts/"+itmStore.strOnUseScript,engGame,false);
//			synchronized(scrStore)
//			{
				scrStore.varVariables.addVariable("trigger",this);
...
//			}
	...
	void castSpell(String spell)
	{
		...
		try
		{
			...
			try
			{
				rafSpell = new RandomAccessFile("/app/defSpells/"+strStore2.toLowerCase(),"r");
			}catch (Exception e)
			{
				chatMessage("You don't know that spell.");
				return;
			...
			try
			{
				scrSpell = new Script("/app/defSpellGroups/"+intStore2+" "+strStore.toLowerCase(),engGame,false);
			}catch(Exception e)
			{
				chatMessage("You don't know that spell.");
				return;
	...
	void runWearScript(Item itmStore)
	{
		if (itmStore.strOnWearScript != null)
		{
			try
			{
				Script scrStore = new Script("/app/scripts/"+itmStore.strOnWearScript,engGame,false);
				scrStore.varVariables.addVariable("trigger",this);
				scrStore.varVariables.addVariable("itemname",itmStore.strName);
				scrStore.varVariables.addVariable("mod",itmStore.intMod);
				scrStore.runScript();
	...
	void runUnWearScript(Item itmStore)
	{
		if (itmStore.strOnUnWearScript != null)
		{
			try
			{
				Script scrStore = new Script("/app/scripts/"+itmStore.strOnUnWearScript,engGame,false);
				scrStore.varVariables.addVariable("trigger",this);
				scrStore.varVariables.addVariable("itemname",itmStore.strName);
				scrStore.varVariables.addVariable("mod",itmStore.intMod);
				scrStore.runScript();
	...
	void addCondition(Condition cndStore)
	{
			...
			while (i<vctConditions.size())
			{
				...
				if (cndStore2.strName.equalsIgnoreCase(cndStore.strName))
				{
					...
					if (cndStore2.strOnEnd != null)
					{
						try
						{
							Script scrStore = new Script("/app/scripts/"+cndStore2.strOnEnd,engGame,false);
							scrStore.varVariables.addVariable("trigger",this);
							scrStore.runScript();
							scrStore.close();
						}catch(Exception e)
		...
		if (cndStore.strOnStart != null)
		{
			try
			{
				Script scrStore = new Script("/app/scripts/"+cndStore.strOnStart,engGame,false);
				scrStore.varVariables.addVariable("trigger",this);
		...
		for (int i = 0;i<vctConditions.size();i++)
		{
			...
			if (strName.equalsIgnoreCase(cndStore.strName))
			{
				...
				if (cndStore.strOnEnd != null)
				{
					try
					{
						Script scrStore = new Script("/app/scripts/"+cndStore.strOnEnd,engGame,false);
						scrStore.varVariables.addVariable("trigger",this);
						scrStore.varVariables.addVariable("condition",cndStore.strName);
						scrStore.runScript();
						scrStore.close();
	...
	boolean getPlayer()
	{
	    ...
	    try
		{
			File filStore = new File("/app/users/"+strName.toLowerCase());
			if (!filStore.exists() || filStore.length() < 100)
			{
				File filBackup = new File("/app/users/"+strName.toLowerCase()+".backup");
				if (filBackup.exists())
				{
			...
			if (filStore.exists())
			{
				try
				{
					rafFile = new RandomAccessFile("/app/users/"+strName.toLowerCase(), "r");
			...
			{
				File fileStore = new File("/app/pets/"+strName.toLowerCase());
				if (fileStore.exists())
				{
			...
			try
			{
				...
				proceed();
				chatMessage("This game is running DuskServer v"+engGame.version+". https://duskrpg.blogspot.com/");
				...
				rafFile = new RandomAccessFile("/app/users/default","r");
				strStore = rafFile.readLine();
				while (!(strStore == null || strStore.equals(".")))
				{
					parseUserFile(strStore);
	    	...
	    	strStore = strName.toLowerCase();
			File filPlayer = new File("/app/users/"+strStore);
			File filBackup = new File("/app/users/"+strStore+".backup");
			File filCheck;
			int i2=0;
			if (filBackup.exists())
			{
				if (filPlayer.length() > filBackup.length())
				{
					...
					{
						i2++;
						filCheck = new File("/app/backup/"+strStore+".possiblyDamaged."+i2);
					}
					filBackup.renameTo(filCheck);
				}else if (filPlayer.length() < filBackup.length())
				{
					...
					filPlayer.renameTo(filCheck);
					filBackup.renameTo(new File("/app/users/"+strStore));
				}
			}
			/*
			** Load the user
			*/
			rafFile = new RandomAccessFile("/app/users/"+strStore, "rw");
			rafBackup = new RandomAccessFile("/app/users/"+strStore+".backup", "rw");
	    	strStore = rafFile.readLine();
			while (!(strStore == null || strStore.equals(".")))
			{
				parseUserFile(strStore);
	...
	void loadRace()
	{
		...
		if (isPet())
		{
			strRaceDir = "/app/defPets";
		}
		else
		{
			strRaceDir = "/app/defRaces";
		}
		try
		{
